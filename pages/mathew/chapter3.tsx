...
export default function Chapter1() {
  ...
  const chapter: string = "chapter_3"
  ...
  return (
    <div className={styles.container}>
      ...
      <div>
        {array.map((a: string) => (<div>{ParagraphTemplate(htmlToString(<HtmlText text={text[author][chapter][`paragraph_${a}`]}/>), htmlToString(<HtmlEnglishText text={textEnglish[author][chapter][`paragraph_${a}`]}/>), `${a}`, englishText)}</div>))}
      </div>
    </div>)
};